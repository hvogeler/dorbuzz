...
static const char *TAG = "dbz-wifi";
                              ...
                              int32_t event_id, void *event_data)
{
    ...
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED)
    {
        ...
        state.set_is_wlan_connected(false);
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        auto &state = State::getInstance();
        std::lock_guard<std::mutex> lock_state(state.getMutex());
        state.set_is_wlan_connected(true);
        wifi.is_connected = true;
    }
}
...
{
    setenv("TZ", CONFIG_DORBUZZ_TIMEZONE, 1);
    tzset();

    esp_sntp_setoperatingmode(SNTP_OPMODE_POLL);
    esp_sntp_setservername(0, CONFIG_DORBUZZ_TIME_SERVER_0);
    esp_sntp_setservername(1, CONFIG_DORBUZZ_TIME_SERVER_1);
    esp_sntp_init();
