...
public:
    ...
    {
        std::string topic_base = CONFIG_DORBUZZ_LOG_TOPIC;
        std::string device_id = "one";
        topic = topic_base + device_id + "/";
    };

    std::string get_esp_localtime();
    void publish_wrapper(std::string, std::string);
...
template <typename... Args>
void MqttLogger::make_log(Severity severity, std::string code_loc, const char *format, Args... args)
{
    ...
    cJSON_AddStringToObject(json, "device_id", "one");
    ...
    publish_wrapper(full_topic, std::string(json_str));
    cJSON_Delete(json);
}

template <typename... Args>
void MqttLogger::info(std::string code_loc, const char *format, Args... args)
{
    make_log(Severity::Info, code_loc, format, args...);
}

template <typename... Args>
void MqttLogger::error(std::string code_loc, const char *format, Args... args)
{
    make_log(Severity::Error, code_loc, format, args...);
}

template <typename... Args>
void MqttLogger::warning(std::string code_loc, const char *format, Args... args)
{
    make_log(Severity::Warning, code_loc, format, args...);
}

template <typename... Args>
void MqttLogger::debug(std::string code_loc, const char *format, Args... args)
{
    make_log(Severity::Debug, code_loc, format, args...);
}

template <typename... Args>
void MqttLogger::trace(std::string code_loc, const char *format, Args... args)
{
